...
Mirrors the `--extract-illustrations` block in pdomain-ocr-cli/ocr_to_txt.py:503-526:
...
def _map_region_type(rt: Any) -> str:
    ...
    if not _REGION_TYPE_MAP:
        try:
            from pdomain_book_tools.layout.types import RegionType  # pyright: ignore[reportMissingImports]
...
def auto_detect_illustrations(
    ...
    *,
    ...
    try:
        from pdomain_book_tools.layout.types import (  # pyright: ignore[reportMissingImports]
            LayoutRegion,
            RegionType,
        )
    except ImportError as exc:
        raise RuntimeError("pdomain_book_tools layout types are not available") from exc
